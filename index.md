...
Imagine an expert who, given an input, is supposed to make a binary decision and report their prediction as a single probability value. This expert considers two types of data when making their decision: the input itself, and a set of prediction probabilities for this same decision from a panel of other experts. The expert then performs the following steps to arrive at their own decision:
...
3. By comparing their prediction with the expected output, the expert adjusts the weights which have been used to arrive at their prediction, by shifting them slightly in the direction which encourages the correct decision in the future.
