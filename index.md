...
weights = np.full(
                shape=(2**context_map_size, previous_layer_output_size),
                fill_value=1/previous_layer_output_size) # array of weights for the Neuron, initialised uniformly as suggested in the paper
...
A neuron takes the probabilistic predictions of the previous layer and combines them using the context-selected weights to produce a prediction of its own. This is implemented as **geometric mixing**, that is, a dot product of the weights and the prediction logits of the previous layer and subsequent sigmoid: ![formula](https://bit.ly/2YwjFwH)

where ![formula](https://bit.ly/2YABYkl) signifies the mixing output when the ground truth is `1`.
...
- If all weights have the uniform value ![formula](https://bit.ly/2CZnhzC) (`d` being the number of neurons in the previous layer), i.e. equal weighting, then the output is the geometric mean of the predictions of the previous layer.
...
We find the loss for the sigmoid output in the standard way, taking the difference between the output and the ground truth and multiplying it with the logit of the output ![formula](https://bit.ly/3jdPZME)
